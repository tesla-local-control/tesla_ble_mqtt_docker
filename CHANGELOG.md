...
- NEW Features:
   ...
   - Environment Variable $AUTO_UPDATE. If this is set to true, then after a command has been sent to the car, a state command will be sent to automatically update the state_topic for the relevent entity. If set to false (default), this doesn't happen automatically, the state_topic is updated at the next polling occurence or a force update button press. See https://github.com/tesla-local-control/tesla_ble_mqtt_docker/issues/82
