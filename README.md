...
The original production release didn't poll to read state. You (or more likely a HA automation) needed to press the 'Force Data Update' button to read / update the state. This development version adds the option of automatic polling of state. Note the following:
 ...
 - The environment variable $AUTO_UPDATE enables or disables the automatic updating of state_topics after a command to change a value has been sent. If true, a state command is sent automatically to update the state_topic. If false (default), the state_topic is updated at the next polling event or force update button press. Setting to true will increase load on the bluetooth system which may cause problems for some configurations
